...
import { createAuthTestStub, type AuthTestStub } from "./helpers/authTestStub";

let server: Server;
let baseUrl: string;
let authStub: AuthTestStub;
let adminCookie: string;
let analystCookie: string;
let viewerCookie: string;
let originalQuery: typeof appDb.query;

async function call(method: string, path: string, { cookie }: { cookie?: string } = {}): Promise<{ status: number; payload: unknown }> {
...
function notDeniedByPolicy(status: number): void {
